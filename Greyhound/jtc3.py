...
def split_anz_horse_market_name(market_name: str) -> Tuple[str, str, str]:
    ...
    # Handle cases where market name doesn't have expected format
    if len(parts) < 3:
        return ('', '', '')
...
def filter_market(market: MarketBook) -> bool: 
    ...
    # Check country and market type first
    if d.country_code != 'AU' or d.market_type != 'WIN':
        return False
    
    # Parse the race type
    race_type = split_anz_horse_market_name(d.name)[2]
    
    # Return False if race type is empty (malformed name), trot, or pace
    if not race_type or race_type == 'trot' or race_type == 'pace':
        return False
    
    return True

# Helper function for markets that go in play
def runner_vals_inplay(pre_ltp, pre_traded, post_ltp, post_traded, sp_traded):
...
# Helper function for markets that don't go in play
def runner_vals_no_inplay(ltp, traded, sp_traded):
...
with open(file_output, "w") as output:
    ...
    for i, file_obj in enumerate(load_markets(market_paths)):
        ...
        # Build postplay_traded with safe SP calculation
        postplay_traded = []
        for r in postplay_market.runners:
            # Calculate SP traded volume safely
            sp_traded = 0
            if r.sp.actual_sp is not None:
                bsp = r.sp.actual_sp if (type(r.sp.actual_sp) is float) or (type(r.sp.actual_sp) is int) else 0
                # Only calculate if BSP is valid (> 1)
                if bsp > 1:
                    back_stake = next((pv.size for pv in r.sp.back_stake_taken if pv.size > 0), 0)
                    lay_liability = next((pv.size for pv in r.sp.lay_liability_taken if pv.size > 0), 0)
                    sp_traded = min_gr0(back_stake, lay_liability / (bsp - 1))
            
            postplay_traded.append((r.last_price_traded, r.ex.traded_volume, sp_traded))
        ...
        if preplay_traded is not None:
            runner_traded = [ 
                runner_vals_inplay(pre_ltp, pre_traded, post_ltp, post_traded, sp_traded)
                for (pre_ltp, pre_traded), (post_ltp, post_traded, sp_traded) in zip(preplay_traded, postplay_traded)
            ]
        # runner price data for markets that don't go in play
        else:
            runner_traded = [ 
                runner_vals_no_inplay(ltp, traded, sp_traded)
                for (ltp, traded, sp_traded) in postplay_traded
            ]
